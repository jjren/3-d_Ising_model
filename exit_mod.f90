...
subroutine exit_Ising(exit_mode, exit_message)
...
! print message
	if (myid == master_id) then
		...
		case(sigExit)
			write (*,'(a14)') 'Ising exiting...'
		case(sigAbort)
			write (*,'(a15)') 'Ising aborting...'
		case default
			write (*,'(a37)') 'Ising exiting with unknown exit mode...'
		end select
